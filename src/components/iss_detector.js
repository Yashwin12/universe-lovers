...
export default class Iss_detector extends React.Component {
    ...
    componentDidMount() {
        ...
        this.state = setInterval(()=> axios.get("http://localhost:3002/temp1").then(res=>console.log(res.data)), 3000) // Every 30 seconds the path of the ISS is updateed.
    }
    ...
    render(){
        ...
        return (
            <Viewer full>
                ...
                <Entity
                    position={Cartesian3.fromDegrees(parseFloat(this.state.long), parseFloat(this.state.lat), 100)}
                    billboard = {{image, height: 60, width: 60}}
                >
